...
static size_t webSocketSendFrameHeader(AsyncClient *client, bool final, uint8_t opcode, uint8_t mask_buf[4], size_t len){
  ...
  size_t sent = client->add((const char *)buf, headLen, TCP_WRITE_FLAG_COPY | TCP_WRITE_FLAG_MORE);
  if(sent != headLen){
...
static size_t webSocketSendFrame(AsyncClient *client, bool final, uint8_t opcode, bool mask, uint8_t *data, size_t len){
  ...
  if(len){
    ...
    added = client->add((const char *)data, len, TCP_WRITE_FLAG_COPY);
  }
  client->send();
  return added;
}
...
static size_t sendListData(AsyncClient* client, std::list<SharedBuffer>& list, size_t& offset, size_t len) {
  ...
  while ((len > 0) && !list.empty()) {
    ...
    auto added = client->add(buf.data() + offset, to_write, ((to_write != len) ? TCP_WRITE_FLAG_MORE : 0) | TCP_WRITE_FLAG_COPY);
    len -= added;
    sent += added;
    offset += added;
    if (offset == buf.size()) {
